...
namespace CompensationSystemSubInterface {
    ...
    /// 员工信息查询高级筛选条件窗体
    /// </summary>
    public partial class FrmEmpCondition : Form {
        /// <summary>
        /// 获取当前的筛选条件
        /// </summary>
        public EmpQueryCondition CurrentCondition { get; private set; }

        /// <summary>
        /// 定义事件: 点击应用时将最新的条件传给主界面
        /// </summary>
        public event Action<EmpQueryCondition> ApplySelect;

        /// <summary>
        /// 初始化薪资筛选条件窗体
        /// </summary>
        /// <param name="existing">现有的筛选条件，如果为 null 则创建新的筛选条件对象</param>
        public FrmEmpCondition(EmpQueryCondition existing) {
            InitializeComponent();
            CurrentCondition = existing != null ? existing.Clone() : new EmpQueryCondition();
        }

        /// <summary>
        /// 窗体加载事件处理
        /// </summary>
        private void FrmEmpCondition_Load(object sender, EventArgs e) {
            LoadBaseData();
            RestoreSelection();
