...
namespace CompensationSystemSubInterface {
    ...
    /// 薪资查询高级筛选条件窗体
    /// </summary>
    public partial class FrmSalaryCondition : Form {
        /// <summary>
        /// 获取当前的筛选条件
        /// </summary>
        ...
        public FrmSalaryCondition(SalaryQueryCondition existing) {
            InitializeComponent();
        ...
        /// 窗体加载事件处理
        /// </summary>
        private void FrmSalaryCondition_Load(object sender, EventArgs e) {
            LoadBaseData();
            RestoreSelection();

            // 绑定右键菜单 (防止Designer未绑定)
            BindContextMenu(clbSeq);
            BindContextMenu(clbDept);
            BindContextMenu(clbPost);
            BindContextMenu(clbLevel);
            BindContextMenu(clbEmp);
            BindContextMenu(clbItem);
        }
        ...
        private void LoadBaseData() {
            ...
            BindList(clbSeq, "SELECT id, xlname FROM ZX_config_xl WHERE IsEnabled=1 AND DeleteType=0", "xlname", "id");
            BindList(clbPost, "SELECT id, gwname FROM ZX_config_gw WHERE IsEnabled=1 AND DeleteType=0 ORDER BY DisplayOrder", "gwname", "id");
            BindList(clbLevel, "SELECT id, cjname FROM ZX_config_cj WHERE DeleteType=0", "cjname", "id");
            BindList(clbEmp, "SELECT id, xingming FROM ZX_config_yg WHERE zaizhi=1 ORDER BY xuhao", "xingming", "id");
            //BindList(clbItem, "SELECT ItemId, ItemName FROM ZX_SalaryItems WHERE IsEnabled=1 ORDER BY DisplayOrder ASC", "ItemName", "ItemId");
            BindList(clbItem, "SELECT ItemId AS id, ItemName FROM ZX_SalaryItems WHERE IsEnabled=1 ORDER BY DisplayOrder ASC", "ItemName", "id");
        }
        ...
        private void RestoreSelection() {
            ...
            SetChecks(clbSeq, CurrentCondition.SequenceIds);
            SetChecks(clbPost, CurrentCondition.PositionIds);
            SetChecks(clbLevel, CurrentCondition.LevelIds);
            SetChecks(clbEmp, CurrentCondition.EmployeeIds);
            SetChecks(clbItem, CurrentCondition.SalaryItemIds);
        }
        ...
        private void btnConfirm_Click(object sender, EventArgs e) {
            ...
            CurrentCondition.SequenceIds = GetChecks(clbSeq);
            CurrentCondition.PositionIds = GetChecks(clbPost);
            CurrentCondition.LevelIds = GetChecks(clbLevel);
            CurrentCondition.EmployeeIds = GetChecks(clbEmp);
            CurrentCondition.SalaryItemIds = GetChecks(clbItem);
